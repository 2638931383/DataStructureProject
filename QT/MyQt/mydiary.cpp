    ...
    , ui(new Ui::myDiary)
{
    ...
    connect(ui->searchLineEdit, &QLineEdit::textChanged, this, &myDiary::onSearchTextChanged);
}
QVector<diaryInfo> myDiary::getAllDiary() {
    QSqlQuery query(db);
    query.prepare("SELECT * FROM diary WHERE userId = :userId");
    query.bindValue(":userId", MySQL_Operate::userID);
    // 执行查询
    if (query.exec()) {
        while (query.next()) {
            diaryInfo info;
...
int myDiary::showPopularityDiaryAll(){
...
int myDiary::showBrowseDiaryAll(){
...
void myDiary::createDiaryEntries() {
    for (const diaryInfo &log : diaryInfoListShow) {
        ...
        connect(button, &QPushButton::clicked, this, &myDiary::showDiaryDetail);
...
void myDiary::updateAllDiaryEntries() {
...
void myDiary::updateDiaryEntries(){
    int i=0;
    int n=diaryInfoListShow.size();
...
void myDiary::showDiaryDetail() {
    ...
    if (button) {
        ...
        myDiaryDetail *detailDiary = new myDiaryDetail(diaryId, this);
        detailDiary->exec();
    }
}
bool myDiary::customContains(const QString &str, const QString &subStr) {
...
int myDiary::searchDiary(const QString &searchInput) {
...
void myDiary::onSearchTextChanged(const QString &text) {
    searchInput = text;
    qDebug()<<searchInput;
    if(searchInput==""){
        isSearch=0;
...
void myDiary::on_allPushButton_clicked()
{
    if(page!=0){
        page=0;
        ui->allPushButton->setStyleSheet(seletcedStyleSheet);
...
void myDiary::on_uploadPushButton_clicked()
{
...
void myDiary::on_popularityPushButton_clicked()
{
    if(page!=1){
        page=1;
...
void myDiary::on_browsePushButton_clicked()
{
    if(page!=2){
        page=2;
        ui->allPushButton->setStyleSheet(initialStyleSheet);
        ...
        ui->searchPushButton->setStyleSheet(initialStyleSheet);
        if(isSearch==0){
            diaryInfoListShow = diaryInfoList;
            showBrowseDiaryAll();
        }
...
void myDiary::on_searchPushButton_clicked()
{
